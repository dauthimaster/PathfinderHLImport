...
- Cannot determine accurate HP formula, if you play a class or archtype that does not use CON for HP you will need to 
    manually modify HP and/or the forumula.
...
- Weapons, the only way to identify "Masterwork" and enhancement bonus (i.e. +1) is if the weapons start with these field,
    therefore weapons made from special materials that are automatically masterwork may not be identified.
...
on("chat:message", function (msg) {
	...
	if (command[0] == "!import") {
        ...
        var LanguagesName = "";
        if (StatBlock.match(/<languages\/>/g) != "<languages\/>") {
            if (StatBlock.match(/<language (.*?)>/g).pop().match(/name='(.*?)'/g) != null) {
                var Languages = StatBlock.match(/<language (.*?)>/g);
