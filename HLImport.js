...
NOTE:  This sheet will only work with Sam's Pathfinder-Neceros style of sheet.

Limitations:
Please be aware that the Hero Lab XML does not directly include all data required to fill out the sheet in the xml file, however with 
that said, I tried to make sure to cover as much of the information as possible.  There simply are some things that were difficult/impossible
to import into Roll20.
- Cannot break down HP, BAB, and Base Saves in the class information section, so all information added to class-0 line.
- Cannot determine any Favorite Class bonuses.
- Cannot handle skill replacement rolls such as the Bard's Versatile Performance, these skills should be updated manually.
- Many bonuses, including but not limited to traits, magic items, or feats are not specified in the xml, therefore all bonuses are entered as misc bonuses.
- Cannot determine accurate HP formula, if you play a class or archtype that does not use CON for HP you will need to manually modify HP and/or the forumula.
- Temporary Hit Points (such as via spell adjustments like false life) are calculated based on Con.
- On monsters with no level (level 0), I attempted to calculate the HitDie of the creature and set it as a level to try to ensure HP|max is correct, but could be error prone.
...
- Many "Special Abilities" do not have a proper classification, doing a random sample, it seemed that the majority were Racial Traits, therefore 
	if you are missing a class ability, you might want to check the Racial Traits area to see if it was copied there.  This is a limitation of the sheet.
- There was no simple way to calculate ability uses or wand charges, so while this information is often in the name of the ability/item, the appropriate fields on the sheet
	will need to be updated manually.
- There is no accurate way to calculate Class Skills per level as this information is not in the xml, therefore it should be manually updated.
- Cannot determine the source of miscellaneous spells that exceed the amount of spells known.
- Spells Known is populated on spontaneous casters purely based on the spells in their known list, this may not accurately represent the rulebook chart in cases where
	bonus spells are added for various reasons.
